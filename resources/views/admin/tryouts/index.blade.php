<x-app-layout>
    ...
    <div class="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <form method="POST" action="{{ route('admin.tryouts.store') }}" class="panel p-6">@csrf
            ...
            <input name="title" class="input-shell admin-input-shell" placeholder="Judul try out">
            <textarea name="description" class="input-shell admin-input-shell" placeholder="Deskripsi try out"></textarea>
            <div class="grid gap-4 sm:grid-cols-3"><input type="number" name="duration_minutes" class="input-shell admin-input-shell" value="60"><input type="number" name="question_count" class="input-shell admin-input-shell" value="20"><select name="difficulty_mix" class="input-shell admin-input-shell"><option value="foundation">Foundation</option><option value="balanced">Balanced</option><option value="challenge">Challenge</option></select></div>
            <textarea name="question_ids" class="input-shell admin-input-shell min-h-28" placeholder="Masukkan ID soal dipisah koma, contoh: 1,2,3,4"></textarea>
