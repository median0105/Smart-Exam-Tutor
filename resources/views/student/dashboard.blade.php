<x-app-layout>
    ...
    <div class="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
        <aside class="panel-dark hidden p-5 text-white xl:block">
            ...
            <nav class="mt-6 space-y-2">
                ...
                <a href="{{ route('student.tryouts.index') }}" class="flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-medium text-slate-300 transition hover:bg-white/10 hover:text-white">
                    <span>Try Out</span>
                    <span>{{ $availableTryouts->count() }}</span>
                </a>
            ...
            <div class="mt-8 rounded-[28px] border border-white/10 bg-white/5 p-5">
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    @forelse($reviewTopics as $topic)
                        <span class="rounded-full border border-white/10 bg-white/10 px-3 py-2 text-xs text-slate-100">{{ $topic }}</span>
                    @empty
        ...
        <div class="space-y-6">
            <section class="panel overflow-hidden p-0">
                <div class="flex flex-col gap-8 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.25),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(139,92,246,0.25),_transparent_28%),linear-gradient(135deg,_#ffffff,_#eef2ff_55%,_#ede9fe)] px-6 py-6 md:px-8 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <p class="text-sm uppercase tracking-[0.35em] text-blue-600">Student Dashboard</p>
                        <h1 class="mt-3 text-3xl font-semibold text-slate-900 md:text-4xl">Belajar dengan tutor yang membaca kebutuhanmu.</h1>
                    ...
                    <div class="min-w-[260px] rounded-[28px] bg-slate-950 p-5 text-white shadow-2xl shadow-violet-500/20">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 {{ $levelBadge }}">{{ $level }}</span>
                        </div>
                        <div class="mt-6">
            ...
            <section class="grid gap-5 sm:grid-cols-2 2xl:grid-cols-4">
                <div class="stat-card transition hover:-translate-y-1 hover:shadow-xl">
                    <p class="text-sm text-slate-500">Total try out dikerjakan</p>
                ...
                <div class="stat-card transition hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <p class="mt-3 text-xs text-slate-500">Hasil klasifikasi try out berbasis KNN.</p>
                </div>
                <div class="stat-card transition hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <p class="mt-2 text-xs text-slate-500">{{ $reviewTopics->join(', ') ?: 'Belum ada topik prioritas' }}</p>
                </div>
            </section>
            ...
            <section class="grid gap-6 2xl:grid-cols-[1.15fr_0.85fr]">
                <div class="space-y-6">
                    <div class="panel p-6">
                        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            ...
                            <div class="grid gap-3 sm:grid-cols-3">
                                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                                    <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Best</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-900">{{ $performanceSummary['best_score'] }}</p>
                                </div>
                                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                                    <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Latest</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-900">{{ $performanceSummary['latest_score'] }}</p>
                                </div>
                                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                                    <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Consistency</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-900">{{ $performanceSummary['consistency'] }}%</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 grid items-end gap-3 sm:grid-cols-5">
                            ...
                            @empty
                                <p class="text-sm text-slate-500">Grafik akan muncul setelah try out pertama selesai.</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="panel p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">Materi dipilih berdasarkan kemiripan topik kelemahan Anda.</p>
                            </div>
                            <a href="{{ route('student.recommendations') }}" class="rounded-full bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-100">Lihat detail</a>
                        </div>
                        ...
                        <div class="mt-5 grid gap-4 md:grid-cols-2">
                            @forelse($materialRecommendations as $recommendation)
                                <div class="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                                    <div class="flex items-center justify-between">
                                        <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">Materi</span>
                                        <span class="text-xs font-medium text-slate-500">Skor {{ $recommendation->similarity_score }}</span>
                                    </div>
                                    <h3 class="mt-4 text-lg font-semibold text-slate-900">{{ $recommendation->recommendable->title ?? 'Materi belajar' }}</h3>
                                    <p class="mt-2 text-sm leading-6 text-slate-600">{{ $recommendation->reason }}</p>
                                </div>
                            @empty
                                <div class="rounded-[28px] border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500 md:col-span-2">
                                    Materi rekomendasi akan muncul setelah hasil try out dianalisis.
                                </div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                        ...
                        <div class="mt-5 space-y-4">
                            @forelse($questionRecommendations as $recommendation)
                                <div class="rounded-[28px] border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                                    <div class="flex items-center justify-between">
                                        <span class="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">Latihan soal</span>
                                        <span class="text-xs font-medium text-slate-500">Skor {{ $recommendation->similarity_score }}</span>
                                    </div>
                                    <h3 class="mt-4 text-lg font-semibold text-slate-900">{{ $recommendation->recommendable->topic->name ?? 'Topik latihan' }}</h3>
                                    <p class="mt-2 text-sm leading-6 text-slate-600">{{ $recommendation->reason }}</p>
                                </div>
                            @empty
                    ...
                    <div class="panel p-6">
                        <div class="flex items-center justify-between">
                            <h2 class="text-xl font-semibold text-slate-900">Try out terbaru</h2>
                        ...
                        <div class="mt-5 space-y-4">
                            @forelse($recentAttempts as $attempt)
                                <div class="rounded-[26px] border border-slate-200 p-4 transition hover:bg-slate-50">
                                    <div class="flex items-center justify-between gap-4">
                                        <div>
                                            <p class="font-semibold text-slate-900">{{ $attempt->tryout->title }}</p>
                                            <p class="mt-1 text-sm text-slate-500">{{ $attempt->tryout->subject->name }} / {{ optional($attempt->submitted_at)->format('d M Y H:i') }}</p>
                                        </div>
                            ...
                            @empty
                                <p class="text-sm text-slate-500">Belum ada try out yang selesai dinilai.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
